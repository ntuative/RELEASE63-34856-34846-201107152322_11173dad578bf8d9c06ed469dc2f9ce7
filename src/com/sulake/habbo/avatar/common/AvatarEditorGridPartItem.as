package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_778:IFigurePartSet;
      ...
      private var var_629:Rectangle;
      
      private var var_1772:int = 0;
      
      private var var_438:IAvatarRenderManager;
      ...
      private const const_2342:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1772 = Math.max(this.var_1772,_loc6_.colorLayerIndex);
            }
         }
         this.var_438 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_747 != null && !this.var_1771)
            {
               _loc3_ = this.var_747;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_778)
            {
               switch(this.var_778.clubLevel)
               {
                  case HabboClubLevelEnum.const_52:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_734;
                     break;
                  case HabboClubLevelEnum.const_41:
                     _loc2_.style = HabboIconType.const_881;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1473.color = this.const_2343;
         }
         else
         {
            this.var_1473.color = this.const_2342;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_438.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_438.isFigureReady(_loc1_))
         {
            this.var_438.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_387[_loc6_] + "_" + FigureData.const_163;
               _loc4_ = this.var_438.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_629 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1773)
               {
                  ...
                  this.var_1773 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_629.width,this.var_629.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_629.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_629.y;
               if(this.var_1771 && _loc2_.colorLayerIndex > 0)
               {
