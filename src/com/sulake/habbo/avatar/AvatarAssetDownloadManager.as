package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1129:int = 3;
      
      private var var_501:AssetLoaderStruct;
      
      private var var_1128:Timer;
      ...
      private const const_2379:int = 100;
      
      private const const_1814:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_1128 = new Timer(this.const_2379,1);
         this.var_1128.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_501)
         {
            ...
            this.var_501.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_501)
         {
            ...
            this.var_501.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1129 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1390);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1129);
            if(this.var_1390.indexOf("?") > 0)
            {
               _loc2_ = this.var_1390 + "&retry=" + this.var_1129;
            }
            else
            {
               _loc2_ = this.var_1390 + "?retry=" + this.var_1129;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_2187 = true;
         for each(_loc2_ in this.var_1391)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_3019,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1392[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2187)
         {
            this.var_1391.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         {
            delete this.var_1127[_loc3_];
         }
         while(_loc6_ < this.var_913.length)
         {
            _loc5_ = this.var_913[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_913.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1128.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_914.length > 0 && this.var_913.length < this.const_1814)
         {
            _loc1_ = this.var_914[0];
            _loc1_.startDownloading();
            this.var_913.push(this.var_914.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_914.indexOf(param1) == -1 && this.var_913.indexOf(param1) == -1)
         {
