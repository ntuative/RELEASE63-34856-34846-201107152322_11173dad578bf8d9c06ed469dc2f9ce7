package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2621 = new RoomThumbnailRenderer(param1);
         this.var_1137 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_202 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_202.visible = true;
         Util.hideChildren(this.var_202);
         this.refreshRoomName(this.var_202,this.var_135);
         if(this.var_135.ownerName != "" && this.var_135.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_202,!!this.var_135.event ? this.getEventCtg(this.var_135.categoryId) : this.getRoomCtg(this.var_135.categoryId));
         this.refreshRoomDesc(this.var_202,this.var_135);
         this.refreshExtraCont();
         this.refreshInfo(this.var_202,"trading_allowed",this.var_135.allowTrading);
         this.refreshInfo(this.var_202,"doormode_doorbell",this.var_135.doorMode == 1);
         this.refreshInfo(this.var_202,"doormode_password",this.var_135.doorMode == 2);
         Util.moveChildrenToColumn(this.var_202,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_202.height = Util.getLowestPoint(this.var_202);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_202,this.var_135.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_202.findChildByName("roomowner"));
         if(this.var_135.ownerName != "" && this.var_135.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_202.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1137.refreshTags(_loc1_,this.var_135.tags);
         ...
         this.var_2621.refreshThumbnail(_loc1_,this.var_135.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_280,Tabs.const_411,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
