package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      private var var_467:ISelectorWindow;
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer, param2:ISessionDataManager)
      {
         ...
         super(param1,param2);
         this.var_467 = _window.findChildByName("groups") as ISelectorWindow;
         if(this.var_467)
         {
            _loc3_ = 0;
            while(_loc3_ < this.var_467.numSelectables)
            {
               _loc4_ = this.var_467.getSelectableAt(_loc3_);
      ...
      override public function dispose() : void
      {
         ...
         var _loc2_:* = null;
         super.dispose();
         for each(_loc1_ in this.var_236)
         {
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_119[this.var_602]);
      ...
      {
         var _loc2_:int = this.var_707[this.var_602];
         var _loc3_:Offer = this.var_236[this.var_602][_loc2_];
      ...
      {
         var _loc2_:* = null;
         if(param1 > -1 && param1 < var_514.numGridItems)
         {
            _loc2_ = this.var_236[this.var_602][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_707[this.var_602] = (this.var_236[this.var_602] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_707[0];
         var _loc2_:int = this.var_707[1];
         var _loc3_:int = this.var_707[2];
         var _loc4_:Offer = this.var_236[0][_loc1_];
         var _loc5_:Offer = this.var_236[1][_loc2_];
         var _loc6_:Offer = this.var_236[2][_loc3_];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            if(!(_loc1_.pricingModel == Offer.const_441 || _loc1_.pricingModel == Offer.const_414))
            {
               continue;
            }
            ...
            if(!(_loc2_.productType == ProductTypeEnum.const_64 || _loc2_.productType == ProductTypeEnum.const_69))
            {
               continue;
            }
            ...
            while(this.var_236.length < this._groupNames.length)
            {
               this.var_236.push(new Array());
            }
            switch(_loc4_)
            {
               case "floor":
                  (this.var_236[_loc5_] as Array).push(_loc1_);
                  break;
               case "wallpaper":
                  (this.var_236[_loc5_] as Array).push(_loc1_);
                  break;
      ...
      private function onSelectGroup(param1:WindowEvent) : void
      {
         ...
         if(_loc2_)
         {
            _loc3_ = this.var_467.getSelectableIndex(_loc2_);
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         this.var_467.setSelected(this.var_467.getSelectableByName(param1));
         var _loc2_:int = -1;
         switch(param1)
         {
            case "group.walls":
         ...
         if(_loc2_ > -1)
         {
            ...
            if(var_514)
            {
               var_514.destroyGridItems();
            }
            page.replaceOffers(this.var_236[this.var_602],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_707[this.var_602];
