package com.sulake.habbo.ui
{
   ...
   public class DesktopLayoutManager
   {
      ...
      private static const const_726:String = "room_view";
      
      private static const const_1690:String = "room_widget";
      ...
      public function setLayout(param1:XML, param2:IHabboWindowManager, param3:IHabboConfigurationManager) : void
      {
         ...
         this.var_3095 = param1.copy();
         this.var_101 = param2.buildFromXML(param1,0) as IWindowContainer;
         if(this.var_101 == null)
         {
            throw new Error("Failed to build layout from XML.");
         }
         this.var_101.width = this.var_101.desktop.width;
         this.var_101.height = this.var_101.desktop.height;
         var _loc4_:IWindowContainer = this.var_101.desktop as IWindowContainer;
         _loc4_.addChildAt(this.var_101,0);
         this.var_101.findChildByTag("room_widget_infostand").y = this.var_101.findChildByTag("room_widget_infostand").y - BOTTOM_MARGIN;
         var _loc6_:int = 0;
         while(_loc6_ < this.var_101.numChildren)
         {
            _loc5_ = this.var_101.getChildAt(_loc6_);
            _loc7_ = false || false;
            if(_loc5_.testParamFlag(_loc7_))
            {
               _loc5_.addEventListener(WindowEvent.const_159,this.trimContainer);
            }
            _loc6_++;
         }
      }
      ...
      private function getWidgetContainer(param1:String, param2:IWindow) : IWindowContainer
      {
         ...
         if(param1 == RoomWidgetEnum.const_361)
         {
            return this.var_101.getChildByName("background_widgets") as IWindowContainer;
         }
         var _loc3_:Array = param2.tags;
         var _loc5_:int = 0;
         while(_loc5_ < _loc3_.length)
         {
            if(String(_loc3_[_loc5_]).indexOf(const_1690) == 0)
            {
         ...
         return this.var_101.getChildByTag(_loc4_) as IWindowContainer;
      }
      ...
      public function addRoomView(param1:IWindow) : Boolean
      {
         ...
         var _loc2_:IWindowContainer = this.var_101.getChildByTag(const_726) as IWindowContainer;
         if(_loc2_ == null)
         {
            return false;
         }
      ...
      public function get roomViewRect() : Rectangle
      {
         ...
         var _loc1_:IWindowContainer = this.var_101.findChildByTag(const_726) as IWindowContainer;
         if(!_loc1_)
         {
            return null;
         }
         ...
         _loc2_.offset(this.var_101.x,this.var_101.y);
         return _loc2_;
      }
