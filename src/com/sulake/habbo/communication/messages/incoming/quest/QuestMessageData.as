package com.sulake.habbo.communication.messages.incoming.quest
{
   import com.sulake.core.communication.messages.IMessageDataWrapper;
   
   public class QuestMessageData
   {
       
      
      private var var_1925:String;
      
      private var var_1924:int;
      
      private var var_1926:int;
      
      private var var_2109:int;
      
      private var _id:int;
      
      private var var_1921:Boolean;
      
      private var _type:String;
      
      private var var_2616:String;
      
      private var var_2617:int;
      
      private var var_1923:String;
      
      private var var_2618:int;
      
      private var var_2615:int;
      
      private var var_1552:int;
      
      private var var_1922:Date;
      
      public function QuestMessageData(param1:IMessageDataWrapper)
      {
         this.var_1922 = new Date();
         super();
         this.var_1925 = param1.readString();
         this.var_1924 = param1.readInteger();
         this.var_1926 = param1.readInteger();
         this.var_2109 = param1.readInteger();
         this._id = param1.readInteger();
         this.var_1921 = param1.readBoolean();
         this._type = param1.readString();
         this.var_2616 = param1.readString();
         this.var_2617 = param1.readInteger();
         this.var_1923 = param1.readString();
         this.var_2618 = param1.readInteger();
         this.var_2615 = param1.readInteger();
         this.var_1552 = param1.readInteger();
      }
      
      public function get campaignCode() : String
      {
         return this.var_1925;
      }
      
      public function get localizationCode() : String
      {
         return this.var_1923;
      }
      
      public function get completedQuestsInCampaign() : int
      {
         return this.var_1924;
      }
      
      public function get questCountInCampaign() : int
      {
         return this.var_1926;
      }
      
      public function get activityPointType() : int
      {
         return this.var_2109;
      }
      
      public function get accepted() : Boolean
      {
         return this.var_1921;
      }
      
      public function get id() : int
      {
         return this._id;
      }
      
      public function get type() : String
      {
         return this._type;
      }
      
      public function get imageVersion() : String
      {
         return this.var_2616;
      }
      
      public function get rewardCurrencyAmount() : int
      {
         return this.var_2617;
      }
      
      public function get completedSteps() : int
      {
         return this.var_2618;
      }
      
      public function get totalSteps() : int
      {
         return this.var_2615;
      }
      
      public function get waitPeriodSeconds() : int
      {
         if(this.var_1552 < 1)
         {
            return 0;
         }
         var _loc1_:Date = new Date();
         var _loc2_:int = _loc1_.getTime() - this.var_1922.getTime();
         return int(Math.max(0,this.var_1552 - Math.floor(_loc2_ / 1000)));
      }
      
      public function getCampaignLocalizationKey() : String
      {
         return "quests." + this.var_1925;
      }
      
      public function getQuestLocalizationKey() : String
      {
         return this.getCampaignLocalizationKey() + "." + this.var_1923;
      }
      
      public function get completedCampaign() : Boolean
      {
         return this._id < 1;
      }
      
      public function set id(param1:int) : void
      {
         this._id = param1;
      }
      
      public function set accepted(param1:Boolean) : void
      {
         this.var_1921 = param1;
      }
      
      public function get lastQuestInCampaign() : Boolean
      {
         return this.var_1924 >= this.var_1926;
      }
      
      public function get receiveTime() : Date
      {
         return this.var_1922;
      }
      
      public function set waitPeriodSeconds(param1:int) : void
      {
         this.var_1552 = param1;
      }
   }
}
