package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_971:String = "InitConnection";
       
      
      private var var_143:HabboCommunicationDemo;
      ...
      {
         this.var_143.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_143 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_143.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_163,HabboWindowStyle.const_47,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_143.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_2094,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_143.habboConfiguration)
            {
               if(this.var_143.habboConfiguration.getKey("use.sso","RoomObjectVisualizationEnum") == "true")
               {
                  _loc8_.select();
               }
            }
         ...
         if(this.var_143.habboConfiguration && this.var_143.habboConfiguration.getKey("try.existing.session","RoomObjectVisualizationEnum") == "true")
         {
            this.var_1649 = true;
            this._window.visible = false;
            this.windowEventProcessor(WindowEvent.allocate(WindowEvent.const_153,this._window,null,false));
         }
      }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_143.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_143.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_2094,"/");
         ...
         {
            dispatchEvent(new Event(const_971));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_143.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_161.removeEventListener(SSOTicket.const_438,this.onParsedTicketSuccess);
            this.var_161.removeEventListener(SSOTicket.const_384,this.onParsedTicketFailure);
            this.var_161 = null;
         }
         var _loc2_:String = "";
         if(this.var_143.habboConfiguration != null)
         {
            _loc2_ = this.var_143.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_143.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_161 = new SSOTicket(this.var_143.assets,_loc3_,_loc2_,param1);
         this.var_161.addEventListener(SSOTicket.const_438,this.onParsedTicketSuccess);
         this.var_161.addEventListener(SSOTicket.const_384,this.onParsedTicketFailure);
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         {
            this.var_143.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_143.flashClientUrl = this.var_161.flashClientUrl;
         this.var_143.ssoTicket = this.var_161.ticket;
         this.var_161.removeEventListener(SSOTicket.const_438,this.onParsedTicketSuccess);
         this.var_161.removeEventListener(SSOTicket.const_384,this.onParsedTicketFailure);
         this.var_161 = null;
         dispatchEvent(new Event(const_971));
