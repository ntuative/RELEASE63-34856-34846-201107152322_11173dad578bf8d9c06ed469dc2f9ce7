package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_298:IRoomObjectSprite;
      ...
      private var var_867:ColorTransform;
      
      private var var_1634:ColorTransform;
      
      private var var_2052:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_867 = new ColorTransform();
         this.var_867.alphaMultiplier = 1;
         this.var_1634 = new ColorTransform();
         this.var_2052 = new Matrix();
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_2051 >= 0)
         {
            this.var_298 = this._visualization.getSprite(this.var_2051);
            if(this.var_298 && this.var_298.asset)
            {
               if(this.var_298.width <= 1 || this.var_298.height <= 1)
               {
                  return;
               }
               if(this.var_99 == null)
               {
                  this.var_99 = this.var_298.asset.clone();
                  if(this.var_867.alphaMultiplier != 1)
                  {
                     this.var_703 = new BitmapData(this.var_99.width,this.var_99.height,true,4278190080);
                  }
               }
               this.var_2565 = -this.var_298.offsetX;
               this.var_2566 = -this.var_298.offsetY;
               this.var_298.asset = this.var_99;
            }
            if(this.var_99)
            {
            ...
            if(this.var_703)
            {
               this.var_703.fillRect(this.var_703.rect,4278190080);
            }
         }
      }
      ...
      {
         if(!this.var_115 || !this.var_298)
         {
            return;
         }
         if(this.var_99 && this.var_298.asset != this.var_99)
         {
            this.var_298.asset = this.var_99;
         }
         if(this.var_1082)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1082 && this.var_115.hasIgnited)
         {
            this.var_1082 = true;
         }
         _loc8_ = this.var_1251 * this.var_1315;
         this.var_115.update();
         if(this.var_1082)
         {
            if(this.var_115.roomObjectSpriteId >= 0)
            ...
            {
               this.var_99.draw(this.var_703,this.var_2052,this.var_867,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_115.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2565 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1315;
               _loc4_ = this.var_2566 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1315;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1634.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_99.draw(_loc7_,this._translationMatrix,this.var_1634,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2858 = parseInt(param1.@size);
         this.var_2051 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1251 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1315 = this.var_2858 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_867.alphaMultiplier = this._blend;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "RoomObjectVisualizationEnum" ? false : true;
