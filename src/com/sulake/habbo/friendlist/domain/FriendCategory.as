package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      public static const const_458:int = 100;
      ...
      public function addFriend(param1:Friend) : void
      {
         ...
         {
            _loc4_ = this.var_257[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               this.var_257.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      public function getFriendCount(param1:Boolean, param2:Boolean = false) : int
      {
         var _loc4_:* = null;
         var _loc3_:int = 0;
         for each(_loc4_ in this.var_257)
         {
      ...
      public function removeFriend(param1:int) : Friend
      {
         ...
         if(_loc2_ != null)
         {
            Util.remove(this.var_257,_loc2_);
            return _loc2_;
         }
         return null;
      }
      ...
      {
         if(this.var_1095 >= this.getPageCount())
         {
            this.var_1095 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_257.length / const_458);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_1095 * const_458;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_1095 + 1) * const_458,this.var_257.length);
      }
