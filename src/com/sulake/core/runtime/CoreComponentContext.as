package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1600:String = "asset-libraries";
      
      private static const const_1601:String = "library";
      
      private static const const_1596:String = "service-libraries";
      
      private static const const_1598:String = "library";
      
      private static const const_1602:String = "component-libraries";
      
      private static const const_1597:String = "library";
      
      private static const const_1594:String = "error_data";
      
      private static const const_1595:String = "error_cat";
      
      private static const const_1599:String = "error_desc";
       
      
      private var var_402:LibraryLoaderQueue;
      
      private var var_848:IEventDispatcher;
      
      private var var_564:uint;
      
      private var var_401:Function;
      
      private var var_226:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1377:
               debug("Core; using simple frame update handler");
               this.var_401 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_753:
               debug("Core; using complex frame update handler");
               this.var_401 = this.complexFrameUpdateHandler;
               break;
            case Core.const_831:
               debug("Core; using profiler frame update handler");
               this.var_401 = this.profilerFrameUpdateHandler;
               this.var_226 = new Profiler(this);
               attachComponent(this.var_226,[new IIDProfiler()]);
               var_431.addChild(new ProfilerViewer(this.var_226));
               break;
            case Core.const_517:
               debug("Core; using experimental frame update handler");
               this.var_401 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_401 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_564 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_292[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1594,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1595,String(param3));
         ErrorReportStorage.setParameter(const_1599,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_848 = loadingEventDelegate;
         if(this.var_402 == null)
         {
            this.var_402 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1600)[0];
            if(node != null)
            {
               list = node.child(const_1601);
         ...
         try
         {
            node = config.child(const_1596)[0];
            if(node != null)
            {
               list = node.child(const_1598);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1602)[0];
            if(node != null)
            {
               list = node.child(const_1597);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_848)
         {
            this.var_848.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_848)
         {
            ...
            this.var_848.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_848)
                  {
                     this.var_848.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_564 - this.var_402.length,this.var_564,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_226 ? 0 : this.var_1605 & 0;
         if(_loc3_ == Core.const_517)
         {
            this.var_292[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_292[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc4_:int = !!this.var_226 ? 0 : this.var_1605 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_107)
         {
            _loc3_ = this.var_292[_loc5_] as Array;
            if(_loc4_ == Core.const_517)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_401(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_692[priority] = 0;
            receivers = this.var_292[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            {
               if(this.var_692[priority] < priority)
               {
                  ++this.var_692[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_226.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_226)
            {
               detachComponent(this.var_226);
               this.var_226.dispose();
               this.var_226 = null;
            }
            switch(this.var_1605 & 0)
            {
               case Core.const_1377:
                  this.var_401 = this.simpleFrameUpdateHandler;
                  break;
